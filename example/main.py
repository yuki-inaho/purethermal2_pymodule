...
def main():
    with PyPureThermal2() as pt2_camera:
        while True:
            if pt2_camera.update():
                thermal_image = pt2_camera.thermal_image_colorized.copy()
                thermal_image_scaled = cv2.resize(
                    thermal_image, None, fx=SCALE, fy=SCALE
                )
                cv2.imshow("thermal", thermal_image_scaled)
                print(f"Cmax: {pt2_camera.thermal_image_celsius.max()}")
            key = cv2.waitKey(10)

            if key & 0xFF in [ord("q"), 27]:
                break
            if key & 0xFF == ord("s") and "thermal_image_scaled" in locals():
                print("saved")
